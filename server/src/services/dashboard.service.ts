...
export const dashboardService = {
  async getKPIs() {
    ...
    const [
      ...
      // Vendas hoje
      Venda.aggregate([
        { $match: { criadoEm: { $gte: hoje, $lt: amanha }, status: 'finalizada' } },
        { $group: { _id: null, total: { $sum: '$total' }, quantidade: { $sum: 1 } } },
      ]),
      // Vendas do mês
      Venda.aggregate([
        { $match: { criadoEm: { $gte: inicioMes, $lte: fimMes }, status: 'finalizada' } },
        { $group: { _id: null, total: { $sum: '$total' }, quantidade: { $sum: 1 } } },
      ]),
      // Produtos ativos
      ...
      Venda.find({ status: 'finalizada' }).sort({ criadoEm: -1 }).limit(5).select('numero clienteNome total criadoEm'),
    ]);
