...
export const financeiroController = {
  ...
  listCP: asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await financeiroService.listCP(req.query, empresaId);
    res.json({ success: true, ...result });
  }),
  createCP: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const conta = await financeiroService.createCP(req.body, empresaId);
    res.status(201).json({ success: true, data: conta });
  }),
  payCP: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const conta = await financeiroService.payCP(req.params.id as string, empresaId);
    res.json({ success: true, data: conta });
  }),
  deleteCP: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    await financeiroService.deleteCP(req.params.id as string, empresaId);
  ...
  listCR: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const result = await financeiroService.listCR(req.query, empresaId);
    res.json({ success: true, ...result });
  }),
  createCR: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const conta = await financeiroService.createCR(req.body, empresaId);
    res.status(201).json({ success: true, data: conta });
  }),
  receiveCR: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const conta = await financeiroService.receiveCR(req.params.id as string, empresaId);
    res.json({ success: true, data: conta });
  }),
  deleteCR: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    await financeiroService.deleteCR(req.params.id as string, empresaId);
    res.json({ success: true, data: { message: 'Conta removida' } });
  }),
  ...
  listDespesas: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const result = await financeiroService.listDespesas(req.query, empresaId);
    res.json({ success: true, ...result });
  }),
  getDespesa: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const despesa = await financeiroService.getDespesa(req.params.id as string, empresaId);
    res.json({ success: true, data: despesa });
  }),
  createDespesa: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const despesa = await financeiroService.createDespesa(req.body, empresaId);
    res.status(201).json({ success: true, data: despesa });
  }),
  updateDespesa: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const despesa = await financeiroService.updateDespesa(req.params.id as string, req.body, empresaId);
    res.json({ success: true, data: despesa });
  }),
  payDespesa: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const despesa = await financeiroService.payDespesa(req.params.id as string, empresaId);
    res.json({ success: true, data: despesa });
  }),
  deleteDespesa: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    await financeiroService.deleteDespesa(req.params.id as string, empresaId);
  ...
  summary: asyncHandler(async (req: Request, res: Response) => {
    const empresaId = req.user!.empresaId;
    const resumo = await financeiroService.summary(empresaId);
